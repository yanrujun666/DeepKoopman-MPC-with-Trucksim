...
class MPCMain:
    ...
    def __init__(self, root_path: str, param_path: str):
        ...
        # MPC权重矩阵
        # 状态权重（6维：[X, Y, Yaw, vx, vy, yaw_rate]）
        # 目标：速度(vx)优先，同时整体更平顺
        self.tempQ = np.diag([20, 80, 120, 300, 40, 40])
        # Trucksim控制维度为12维（6个转矩+6个转向角）
        # R矩阵（12维增量权重：[6转矩, 6转向角]）
        # 目标：转矩更平滑（增量惩罚更大），转向更昂贵避免抖动
        self.RTimes = np.diag([800, 800, 800, 800, 800, 800, 12000, 12000, 12000, 12000, 12000, 12000])
        # delta_umax：12维控制增量约束
        # 转矩增量：0.05（归一化后，约150 N·m/步），转向角增量：0.02（约3.6deg/步）
        self.delta_umax = np.array([0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02])
